...
static const char* kIsotopeDB = "../Isotope_energys.txt";
    ...
    : TGMainFrame(p, w, h)
{
    ...
    if (db_.Load(kIsotopeDB)) {
        dbLoaded_ = true;
        AppendLog("Isotope DB loaded: " + std::string(kIsotopeDB));
    } else {
        AppendLog("WARNING: Could not load isotope DB from " +
                  std::string(kIsotopeDB) + " — matches will be empty");
    }
