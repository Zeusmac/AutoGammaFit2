...
static std::string ClassToString(int sel, const std::string& custom);
static int         ClassToComboIndex(const std::string& cls);
...
static const char* kResolutionKey      = "__RESOLUTION__";
static const char* kExcludedFwhmKey    = "__EXCLUDED_FWHM__";
static const char* kFwhmPrefix         = "FWHM: ";

static void ApplyHistStyle(TH1* h, const char* title = nullptr) {
...
static void SetYMaxFromVisible(TH1* h, double margin = 1.30) {
...
static std::string BuildNGaussFormula(int n) {
    std::string f;
    for (int i = 0; i < n; i++) {
        if (i) f += "+";
        int p = 3 * i;
...
void GammaFitGUI::BuildIsotopesTab(TGCompositeFrame* p)
{
    ...
    // ── Controls row ──────────────────────────────────────────────────────────
    {
        ...
        TGTextButton* refreshBtn = new TGTextButton(topRow, "Refresh from Cache");
    ...
    p->AddFrame(listGrp, new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 4, 4, 2, 2));

    isoList_ = new TGListBox(listGrp, 960);
    isoList_->Resize(285, 220);
    ...
    }
    {
        ...
        isoCustomLabelEntry_->SetToolTipText(
            "Free-text label saved to cache — overrides the combo above when non-empty");
    ...
    }
    {
        ...
        isoCustomEntry_->SetToolTipText("Custom sub-classification name");
    ...
    editGrp->AddFrame(applyBtn, new TGLayoutHints(kLHintsExpandX, 2, 2, 4, 2));
    applyBtn->Connect("Clicked()", "GammaFitGUI", this, "OnIsoApply()");
    applyBtn->SetToolTipText("Save label + classification to the selected peak's cache entry");

    TGTextButton* applyAllBtn = new TGTextButton(editGrp, "Apply Label to All in Filter");
    editGrp->AddFrame(applyAllBtn, new TGLayoutHints(kLHintsExpandX, 2, 2, 0, 2));
    ...
    editGrp->AddFrame(clearBtn, new TGLayoutHints(kLHintsExpandX, 2, 2, 0, 4));
    ...
    p->AddFrame(dbGrp, new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 4, 4, 4, 4));

    {
        ...
        TGTextButton* clearBtn = new TGTextButton(searchRow, "Clear");
        searchRow->AddFrame(clearBtn, new TGLayoutHints(kLHintsLeft, 4, 0, 0, 0));
        clearBtn->Connect("Clicked()", "GammaFitGUI", this, "OnIsoDbClear()");
        clearBtn->SetToolTipText("Show all database lines");
    }

    isoDbList_ = new TGListBox(dbGrp, 980);
    isoDbList_->Resize(285, 200);
    dbGrp->AddFrame(isoDbList_,
                    new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 2, 2, 0, 2));
    isoDbList_->Connect("Selected(Int_t)", "GammaFitGUI", this,
                        "OnIsoDbLineSelected(Int_t)");

    // Class picker for DB-to-peak assignment
    {
...
void GammaFitGUI::PopulateIsoList(const std::string& filterLabel)
{
    ...
    // Collect one row per fitted PEAK (multi-Gaussian entries expand into N rows).
    // Energy comes from the stored parameters, not the key, so it's the actual
    // fitted centroid even if TSpectrum seeded it at a slightly different value.
    struct Row { std::string key; double energy; std::string label; std::string classification; };
    std::vector<Row> rows;
    for (const auto& kv : fitdb.GetEntries()) {
        ...
        // Well-formed multi-Gaussian: npar = 3*n + 2, n >= 1
        if (npar >= 5 && (npar - 2) % 3 == 0) {
            int n = (npar - 2) / 3;
            for (int i = 0; i < n; i++) {
                double E = fe.params[3*i + 1];  // fitted centroid for peak i
                rows.push_back({kv.first, E, fe.label, fe.classification});
            }
        } else {
            // Fallback: parse energy from key
            double E = 0.0;
    ...
    for (const auto& r : rows) {
        // Best DB match for this peak (used as suggestion when no explicit label)
        std::string autoMatch;
        if (dbLoaded_ && r.label.empty() && r.energy > 0) {
            double fwhm = res_.FWHM(r.energy);
            auto dbM = db_.Match(r.energy, fwhm);
            if (!dbM.empty()) autoMatch = dbM[0].isotope;
        }

        std::string lbl;
        if (!r.label.empty())
            lbl = r.label;                          // confirmed by user
        else if (!autoMatch.empty())
            lbl = autoMatch + "?";                  // auto-suggestion
        else
            lbl = "(unlabeled)";

        std::string cls = r.classification;
        if (cls.empty() && r.energy > 0 && r.energy < 100.0) cls = "X-ray";
        if (cls.empty()) cls = "-";
        std::string entry = Form("%-14s  %-14s  %.1f keV", lbl.c_str(), cls.c_str(), r.energy);
        isoList_->AddEntry(entry.c_str(), id++);
        isoListKeys_.push_back(r.key);
        isoListAutoMatch_.push_back(autoMatch);
    }
...
void GammaFitGUI::RefreshIsoDisplay()
{
    ...
    // Rebuild filter combo from unique labels in cache
    FitDatabase fitdb;
    fitdb.Load(CacheFileFor(isoHistName_));
    ...
    if (isoFilterCombo_) {
        // Preserve current filter selection if still valid
...
void GammaFitGUI::OnIsoListSelected(Int_t id)
{
    ...
    // Auto-match suggestion for this row (computed during PopulateIsoList)
    ...
    // Populate label combo. Priority:
    //   1. confirmed label (from cache) — pre-selected
    //   2. auto-match suggestion — pre-selected when no confirmed label
    //   3. (none)
    if (isoLabelCombo_) {
        ...
        // Add confirmed label at top (if any)
        if (!curLabel.empty())
            isoLabelCombo_->AddEntry(curLabel.c_str(), cid++);

        // Add auto-match at second position if different from confirmed label
        int autoMatchIdx = -1;
        ...
        // Pre-select: confirmed label (idx 2) > auto-match (autoMatchIdx) > (none) (idx 1)
        int selIdx = 1;
        if (!curLabel.empty())
            selIdx = 2;
        else if (autoMatchIdx > 0)
            selIdx = autoMatchIdx;
        isoLabelCombo_->Select(selIdx, kFALSE);
        isoLabelCombo_->MapSubwindows(); isoLabelCombo_->Layout();
    }
    // Populate custom label entry: show stored label if it doesn't match any DB entry
    ...
    if (isoClassCombo_) {
        std::string cls = it->second.classification;
        // Auto-suggest X-ray for unclassified peaks below 100 keV
...
void GammaFitGUI::OnIsoApply()
{
    ...
    if (sel < 1 || (size_t)sel > isoListKeys_.size()) {
        AppendLog("Select a peak from the list first.");
        return;
    }
    ...
    FitEntry e = it->second;
    // Custom label entry takes priority; fall back to combo selection
    {
...
void GammaFitGUI::OnIsoApplyLabelAll()
{
    if (isoHistName_.empty() || isoListKeys_.empty()) {
        AppendLog("Refresh cache first, and ensure the list is not empty.");
        return;
    }
    std::string newLabel;
    if (isoLabelCombo_) {
...
void GammaFitGUI::PopulateIsoDbList(const std::string& filter)
{
    ...
    for (int i = 0; i < (int)db_.db.size(); ++i) {
        ...
        // Match against isotope name or energy string
...
{
    // Require a DB line selected
    Int_t dbId = isoDbList_ ? isoDbList_->GetSelected() : -1;
    if (dbId < 1 || (size_t)dbId > isoDbIndices_.size()) {
        AppendLog("Select a line from the Isotope Database list first.");
        return;
    }
    // Require a fitted peak selected
    Int_t peakId = isoList_ ? isoList_->GetSelected() : -1;
    if (peakId < 1 || (size_t)peakId > isoListKeys_.size()) {
        AppendLog("Select a fitted peak from the Peaks list first.");
        return;
    }
    if (isoHistName_.empty()) {
        AppendLog("Refresh the Isotopes tab for a histogram first.");
        return;
    }
    ...
    std::string newCls;
    if (isoDbClassCombo_) {
        int sel = isoDbClassCombo_->GetSelected();
        newCls = ClassToString(sel, "");
    }
    ...
    const auto& entries = fitdb.GetEntries();
    auto it = entries.find(key);
    if (it == entries.end()) { AppendLog("Cache entry not found for key: " + key); return; }
    ...
    AppendLog("Applied: " + gl.isotope +
              Form("  %.2f keV", gl.energy) +
              (newCls.empty() ? "" : "  [" + newCls + "]") +
              " → peak " + key);
    OnIsoRefresh();
}
...
void GammaFitGUI::OnIsoDbLineSelected(Int_t id)
{
    ...
    // Pre-fill label combo with the selected isotope name
    if (isoLabelCombo_) {
        // Scan existing entries for this isotope
        int found = -1;
...
void GammaFitGUI::BuildFWHMTab(TGCompositeFrame* p)
{
    ...
    fwhmStatLineChk_ = new TGCheckButton(dg,
        "Show stat. + noise limit  [FWHM: sqrt(a+b*E)  /  Res: 100*sqrt(a+b*E)/E]");
    ...
    fwhmStatLineChk_->SetToolTipText(
        "Draw the statistics + noise-floor limit: sqrt(a + b*E).\n"
        "This is the resolution your detector would achieve if the\n"
        "charge-trapping / non-uniformity term (c*E^2) were zero.\n"
        "The gap between this line and your data shows how much\n"
        "the c term degrades resolution at high energies.");
                                    ...
                                    bool showResolution)
{
    ...
    if (!xIn.empty()) {
        ...
        // All excluded: draw invisible frame so axes still appear
    ...
    grIn->SetMarkerStyle(20); grIn->SetMarkerSize(0.8);
    grIn->SetMarkerColor(kBlue + 1); grIn->SetLineColor(kBlue + 1);
    grIn->SetMinimum(0.0);   // y-axis from 0 so the stat limit line is fully visible
    grIn->Draw("AP");
    // Ensure axis labels are visible after Draw
    if (grIn->GetXaxis()) {
    ...
    if (fwhmTF1_) {
        ...
        // Statistical + noise-floor limit: sqrt(a + b*E)
        // This is the resolution you would achieve if the c (charge-trapping)
        // term were zero — i.e. the limit set by electronics noise + counting stats.
        // In Resolution mode: 100 * sqrt(a + b*E) / E
        if (showStatLine) {
            ...
            if (showResolution) {
                ...
                statLine = new TF1("fwhm_stat",
                    Form("sqrt((%.10g+%.10g*x)/%.10g)", a, b, kSig * kSig),
                    0.0, xhi);
            } else {
                statLine = new TF1("fwhm_stat",
                    Form("sqrt(%.10g+%.10g*x)", a, b), 0.0, xhi);
...
void GammaFitGUI::OnFitFWHM()
{
    ...
    std::string result = Form("a=%.4g  b=%.4g  c=%.4g  chi2/ndf=%.2f",
                               a, b, c, chi2ndf);
    fwhmResultLbl_->SetText(result.c_str());
    AppendLog("FWHM fit (" + std::to_string(xIn.size()) + " pts): " + result);
    SetStatus("FWHM fit done  chi2/ndf=" + Fmt(chi2ndf, 2));
